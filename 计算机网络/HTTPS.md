...
2、那有木有一种方式既可以安全的获取公钥，又能防止黑客冒充呢？ 那就需要用到终极武器了：[SSL证书](https://link.zhihu.com/?target=https%3A//www.wosign.com/marketing/ssl_new.htm)

![](..\images\https.jpg)

如上图所示，在第 ② 步时服务器发送了一个SSL(数字签名)证书给客户端，SSL(数字签名) 证书中包含的具体内容有：
...
3、客户端在接受到服务端发来的SSL(数字签名)证书时，会对证书的真伪进行校验，以浏览器为例说明如下：
...
4、所以通过发送SSL(数字签名)证书的形式，既解决了公钥获取问题，又解决了黑客冒充问题，一箭双雕，HTTPS加密过程也就此形成。
...
默认情况下，假的证书颁发机构是不被浏览器信任的，因此必须通过用户手动安装。如果用户不安装这一假证书颁发机构的根证书进而允许劫持，那么用户则完全无法访问受影响的网站。
